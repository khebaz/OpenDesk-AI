...
class ModelManager {
  ...
  download(modelId, progressCb) {
    return new Promise((resolve, reject) => {
      ...
      const url     = `https://huggingface.co/${meta.hfRepo}/resolve/main/${meta.hfFile}`

      const req = https.get(url, { headers: { 'User-Agent': 'OpenDeskAI/0.1' } }, res => {
        if (res.statusCode === 302 || res.statusCode === 301) {
          return https.get(res.headers.location, { headers: { 'User-Agent': 'OpenDeskAI/0.1' } }, res2 => {
            this._streamToFile(res2, tmpDest, dest, modelId, progressCb, resolve, reject)
          }).on('error', reject)
        }
        this._streamToFile(res, tmpDest, dest, modelId, progressCb, resolve, reject)
      })
      req.on('error', err => { if (fs.existsSync(tmpDest)) fs.unlinkSync(tmpDest); reject(err) })
